...
// function checkOrderStatus(symbol, order) {
//   api.Position.Position_get()
//   .then(function(positions){
//     for (var i=0; i<positions.obj.length; i++) {
//       if (positions.obj[i].symbol === symbol) {
//         if (positions.obj[i].currentQty === order.orderQty) {
//           order.ordStatus = "Filled"
//           sendLimitOrderEmail(order)
//         } else {
//           console.log('here')
//           orderChecker = setTimeout(function(){
//             checkOrderStatus(symbol, order)
//           },15000)
//         }
//       }
//     }
...
function placeOrder(client, symbol, side, retry){
        ...
        if (process.env.BITMEX_ORDER_TYPE === "limit") {
          if (totalAmount > 0) {
            ...
            .then(function(order){
              ...
              setTimeout(function(){
                checkOrderStatus(symbol, order.obj);
              },5000)
          ...
          client.Order.Order_new({symbol: symbol, ordType: 'Market', orderQty: totalAmount, side: side})
          .then(function(response){
  ...
  .catch(function(e){
    if (e.obj.error.message.indexOf("system is currently overloaded")) {
      ...
      }, process.env.BITMEX_SYSTEM_OVERLOAD_RETRY * 1000)
